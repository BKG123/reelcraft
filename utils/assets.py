...
async def download_image(photo_id, file_name):
    ...
    file_path = ASSET_FOLDER / f"{file_name}.jpg"
...
async def download_video(video_id, file_name, quality="hd"):
    ...
    if not video_link:
        raise ValueError(
            f"{quality.upper()} video link not found. "
            f"Available qualities: {[vf['quality'] for vf in video_data['video_files']]}"
        )

    file_path = ASSET_FOLDER / f"{file_name}.mp4"
...
async def search_and_download_asset(keyword, asset_type, file_name, **kwargs):
    ...
    """
    if asset_type == "image":
        photo_data = await search_image(keyword, **{k: v for k, v in kwargs.items() if k in ['orientation', 'per_page']})
        ...
        video_data = await search_video(keyword, **{k: v for k, v in kwargs.items() if k in ['orientation', 'per_page']})
